        ...
        <div className="max-w-6xl mx-auto px-6 py-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
                <div className="bg-charcoal border border-gray-800 p-6 rounded-lg hover:border-gray-600 transition-colors">
                    <div className="w-10 h-10 bg-neon-green/10 flex items-center justify-center rounded mb-4">
            ...
            <div className="flex items-center justify-between mb-8">
                ...
                <div className="text-xs font-mono text-gray-500">
                    PROGRESS: <span className="text-neon-green">{watchedModules.length}</span> / {videoModules.length} COMPLETED
                </div>
            </div>
                ...
                {videoModules.map((module) => {
                    ...
                    return (
                        <div 
                            ...
                            className={`group cursor-pointer border rounded-xl overflow-hidden transition-all duration-300 shadow-lg relative ${isWatched ? 'border-neon-green/30 bg-gray-900/10' : 'border-gray-800 bg-gray-900/20 hover:border-neon-green hover:bg-gray-900/40'}`}
                        >
                            {/* Watched Badge */}
                            {isWatched && (
                                <div className="absolute top-3 left-3 z-20 bg-neon-green text-black text-[10px] font-bold px-2 py-1 rounded flex items-center gap-1 font-mono shadow-lg">
                            ...
                            <div className="h-56 bg-gray-900 relative border-b border-gray-800 group-hover:border-neon-green/50 transition-colors">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center transition-colors">
                                    {isWatched ? (
                                        <div className="text-neon-green font-mono text-sm font-bold bg-black/50 px-3 py-1 rounded border border-neon-green/30">
                                            REWATCH
                                        </div>
                                    ) : (
                                        <div className="w-14 h-14 bg-white/10 backdrop-blur-sm border border-white/20 rounded-full flex items-center justify-center group-hover:scale-110 group-hover:bg-neon-green group-hover:text-black group-hover:border-neon-green transition-all shadow-xl">
                            ...
                            <div className="p-6">
                                <div className="flex justify-between items-start mb-3">
                                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded transition-colors ${isWatched ? 'bg-gray-800 text-gray-500' : 'bg-neon-green/10 text-neon-green'}`}>
                                        {module.level}
                                    </span>
                                </div>
                                <h3 className={`text-lg font-bold mb-2 font-sans transition-colors ${isWatched ? 'text-gray-400' : 'text-white group-hover:text-neon-green'}`}>
                                    {module.title}
                                </h3>
